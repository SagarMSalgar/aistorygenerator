...
export const scriptAgent = {
  ...
  async generateScript(dayDescription: string): Promise<ScriptData> {
    try {
      // Attempt to use HuggingFace for script generation first
      const prompt = `
      ...
      try {
        // Try with a different model - mistralai/Mistral-7B-Instruct-v0.2 is better for this task
        ...
        if (summaryMatch && summaryMatch[1]) {
          ...
          // If external API fails for summary, generate one with our internal function
          const dynamicScript = generateDynamicScript(dayDescription);
          summary = dynamicScript.summary;
        }
        ...
        console.log("HuggingFace API failed, using internal script generator:", huggingFaceError);
        // If HuggingFace fails, use our internal generator
        return generateDynamicScript(dayDescription);
      }
    } catch (error) {
      console.error('Error in script generation:', error);
      
      // If all else fails, use our internal generator as a fallback
      return generateDynamicScript(dayDescription);
    }
  }
};
